...
public class AttachmentController extends BaseController {
	...
	public JsonResult deleteAttachment(@RequestParam(value = "id") int id, HttpServletRequest request) {
		...
		try {
			// 获取用户目录
			String userPath = System.getProperties().getProperty("user.home") + "/liawan";
			// 获取文件路径
			...
			if (picturePathFile.isFile() && picturePathFile.exists()) {
				...
				if (pictureSmallPathFile.delete() && picturePathFile.delete()) {
					...
					return new JsonResult(LiawanEnums.OPERATION_ERROR.isFlag(),
							LiawanEnums.OPERATION_ERROR.getMessage());
				}
			}
		} catch (Exception e) {
			log.error("删除文件" + attachment.getPictureName() + "失败");
	...
	public JsonResult uploadAttachment(@RequestParam(value = "file") MultipartFile file, HttpServletRequest request) {
		if (!file.isEmpty()) {
			try {
				...
				attachment.setPictureWh(LiawanUtil.getImageWh(new File(mediaPath.toString() + "/" + fileName)));
				attachment.setPictureSize(LiawanUtil.parseSize(new File(mediaPath.toString() + "/" + fileName).length()));
